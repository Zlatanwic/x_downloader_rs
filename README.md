# 一个基于Rust的X平台帖子视频下载工具
目前只有linux系统的可执行文件以及代码
## 配置方法
需要自行下载linux版本的yt-dlp可执行文件和ffmpeg可执行文件，放在本程序x_downloader_rs可执行文件的同目录下:
首先下载yt-dlp
```shell
# 1. 下载 Linux 版的可执行文件，并重命名为 yt-dlp (没有 .exe 后缀)
wget https://github.com/yt-dlp/yt-dlp/releases/latest/download/yt-dlp

# 2. 赋予执行权限 (这一步必须做，否则 Linux 不让运行)
chmod +x yt-dlp
```
然后下载ffmpeg
```shell
# 1. 下载压缩包 (tar.xz 格式)
wget https://johnvansickle.com/ffmpeg/releases/ffmpeg-release-amd64-static.tar.xz

# 2. 解压这个包
tar -xf ffmpeg-release-amd64-static.tar.xz

# 3. 把里面的 ffmpeg 和 ffprobe 移到当前目录
# (注意：解压出来的文件夹名字可能稍有不同，这里用通配符 *)
mv ffmpeg-*-static/ffmpeg .
mv ffmpeg-*-static/ffprobe .

# 4. 清理掉没用的压缩包和文件夹
rm -rf ffmpeg-release-amd64-static.tar.xz ffmpeg-*-static

# 5. 再赋予一下执行权限
chmod +x ffmpeg ffprobe
```
并且需要你有自己的x账号，并在运行本程序前保持登录状态，并且在浏览器插件商店里找到“Get cookies.txt LOCALLY”插件，在x页面选择export，并且把输出的txt文件改成"cookies.txt"，也放在同目录下
## 用法
```shell
./x_downloader_rs "url(替换为你想要的x帖子链接)"
```
## 保存
下载好的视频保存在download文件夹中
## 注意
1. 使用的链接必须是有效的视频帖子链接，意味着不能是图片的帖子链接以及跳转到别的网站的链接
2. linux环境下(windows用户可以使用wsl2)需要在x_downloader_rs前加"./"

## 免责声明
本工具原则上只支持正规视频下载，如果用于其他类型视频下载，与本项目无关，概不负责
